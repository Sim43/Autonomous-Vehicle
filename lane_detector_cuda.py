...
class LaneDetector:
    ...
    def _compute_perspective(self):
        ...
        return cv2.getPerspectiveTransform(cp.asnumpy(src), cp.asnumpy(dst)), cv2.getPerspectiveTransform(cp.asnumpy(dst), cp.asnumpy(src))
    ...
    def abs_sobel_thresh(self, sobel_gpu, thresh=(0, 255)):
        ...
        scale = float(cp.max(sobel_abs)) if cp.max(sobel_abs) > 0 else 1.0
        scaled_sobel = cp.uint8(255 * sobel_abs / scale)
    ...
    def mag_threshold(self, sobelx_gpu, sobely_gpu, thresh=(0, 255)):
        ...
        scale = float(cp.max(mag)) / 255 if cp.max(mag) > 0 else 1
    ...
    def hls_select(self, img_gpu, sthresh=(0, 255), lthresh=(0, 255)):
        ...
        hls = cp.asarray(hls_gpu.download())
        l_channel = hls[:, :, 1]
        s_channel = hls[:, :, 2]
        binary_output = cp.zeros_like(s_channel)
    ...
    def binary_pipeline(self, img):
        ...
        combined = cp.zeros_like(s_binary)
        combined[((x_binary == 1) & (y_binary == 1)) | ((mag_binary == 1) & (dir_binary == 1))] = 1

        final_binary = cp.bitwise_or(s_binary, combined)
        return cp.asnumpy(final_binary)
    ...
    def track_lanes_initialize(self, binary_warped):
        ...
        for window in range(nwindows):
            ...
            if len(good_left_inds) > minpix:
                leftx_current = int(cp.mean(nonzerox[good_left_inds]))
            if len(good_right_inds) > minpix:
                rightx_current = int(cp.mean(nonzerox[good_right_inds]))

        left_lane_inds = cp.concatenate(left_lane_inds)
        right_lane_inds = cp.concatenate(right_lane_inds)
    ...
    def lane_fill_poly(self, binary_warped, undist, left_fit, right_fit):
        ploty = cp.linspace(0, binary_warped.shape[0]-1, binary_warped.shape[0])
        left_fitx = self.get_val(ploty, left_fit)
        right_fitx = self.get_val(ploty, right_fit)

        warp_zero = cp.zeros_like(binary_warped).astype(cp.uint8)
        color_warp = cp.dstack((warp_zero, warp_zero, warp_zero))

        pts_left = cp.array([cp.transpose(cp.vstack([left_fitx, ploty]))])
        pts_right = cp.array([cp.flipud(cp.transpose(cp.vstack([right_fitx, ploty])))])
        pts = cp.hstack((pts_left, pts_right))

        color_warp_cpu = cp.asnumpy(color_warp)
        pts_cpu = cp.asnumpy(pts)

        cv2.fillPoly(color_warp_cpu, [pts_cpu.astype(int)], (0,255,0))
        newwarp = cv2.warpPerspective(color_warp_cpu, self.inverse_perspective_transform, (binary_warped.shape[1], binary_warped.shape[0]))
